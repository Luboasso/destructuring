...
// {"name":"Ana", "email":"Ana@gmail.com"}
const datosAna = empleados.filter(empleado => empleado.name == "Ana")
// console.log(datosAna);


// <!-- Extrae el email del empleado Luis --> Luis@gmail.com -->
const datosLuis = empleados.filter(empleado => empleado.name == "Luis")
...
// Escribe una función llamada sumEveryOther que pueda recibir cualquier cantidad de números y devuelva la suma de todos los demás argumentos.
  ...
  const sumNums = nums.reduce((a,b) => a + b)
console.log(sumNums)
  }
...
const sumOnlyNums =  nums.filter((nums) => typeof nums === "number")
return sumOnlyNums.reduce((a,b) => a + b)
...
const array1 = ['gato', 'perro', 'conejo'];
const array2 = ['delfin', 'canario'];
...
const onlyUniques = (...stuff) => {
    const argumentos = new Set(stuff);
    return [...argumentos]
}
...
const sumAndSquare = (...arr) => {
  const squared = arr.map((value) => value ** 2);
 const sum = squared.reduce((a, b) => a + b)
return sum
} ;
console.log(sumAndSquare(2, 3, 6))